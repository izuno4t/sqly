...
- **SQL ファースト** — ORM ではなく SQL を直接書く。sqlym は SQL を
  自動生成しない
...
| RDBMS 別 SQL ファイルロード | `find.oracle.sql` → `find.sql` のフォールバック |
...
sqlym は SQL テンプレートエンジンであり、以下の機能は提供しません。
...
sqlym の 2way SQL パーサーは、tauty 氏の
