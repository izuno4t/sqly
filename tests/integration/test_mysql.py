"""MySQL 統合テスト: パース → DB実行 → マッピングの一連フロー検証."""
...
@pytest.fixture
def db_aliased(mysql_conn: Any) -> Any:
...
class TestBasicFlow:
    ...
    def test_select_all(self, db: Any) -> None:
        ...
        result = parse_sql(sql, {}, dialect=Dialect.MYSQL)
    ...
    def test_select_with_param(self, db: Any) -> None:
        ...
        result = parse_sql(sql, {"id": 2}, dialect=Dialect.MYSQL)
        rows = _fetch_all(db, result)
        mapper = create_mapper(Employee)
        employees = mapper.map_rows(rows)
...
class TestLineRemoval:
    ...
    def test_none_param_removes_condition(self, db: Any) -> None:
        ...
        sql = "SELECT * FROM employees\nWHERE\n    name = /* $name */'default'"
        result = parse_sql(sql, {"name": None}, dialect=Dialect.MYSQL)
    ...
    def test_partial_removal(self, db: Any) -> None:
        ...
        result = parse_sql(sql, {"dept_id": 10, "name": None}, dialect=Dialect.MYSQL)
        rows = _fetch_all(db, result)
        mapper = create_mapper(Employee)
        employees = mapper.map_rows(rows)
    ...
    def test_all_conditions_none(self, db: Any) -> None:
        ...
        result = parse_sql(sql, {"dept_id": None, "name": None}, dialect=Dialect.MYSQL)
        assert "WHERE" not in result.sql
        rows = _fetch_all(db, result)
        assert len(rows) == 4
...
class TestInClause:
    ...
    def test_in_clause_list(self, db: Any) -> None:
        ...
        result = parse_sql(sql, {"ids": [1, 3]}, dialect=Dialect.MYSQL)
        rows = _fetch_all(db, result)
        mapper = create_mapper(Employee)
        employees = mapper.map_rows(rows)
        assert len(employees) == 2
    ...
    def test_in_clause_single(self, db: Any) -> None:
        """IN 句で単一要素リスト."""
        sql = "SELECT * FROM employees WHERE id IN /* $ids */(999)"
        result = parse_sql(sql, {"ids": [2]}, dialect=Dialect.MYSQL)
        rows = _fetch_all(db, result)
        assert len(rows) == 1
    ...
    def test_in_clause_empty_list(self, db: Any) -> None:
        """IN 句で空リスト → NULL → 0件."""
        sql = "SELECT * FROM employees WHERE id IN /* $ids */(999)"
        result = parse_sql(sql, {"ids": []}, dialect=Dialect.MYSQL)
        rows = _fetch_all(db, result)
        assert len(rows) == 0
...
class TestEntityDecorator:
    ...
    def test_naming_convention_mapping(self, mysql_conn: Any) -> None:
        ...
        result = parse_sql(sql, {}, dialect=Dialect.MYSQL)
        rows = _fetch_all(mysql_conn, result)
...
class TestSqlLoaderIntegration:
    ...
    def test_load_and_execute(self, db: Any, tmp_path: Path) -> None:
        ...
        result = parse_sql(sql_template, {"dept_id": 10}, dialect=Dialect.MYSQL)
        rows = _fetch_all(db, result)
        mapper = create_mapper(Employee)
        employees = mapper.map_rows(rows)
        assert len(employees) == 2
    ...
    def test_load_with_none_param(self, db: Any, tmp_path: Path) -> None:
        ...
        result = parse_sql(sql_template, {"dept_id": None, "name": "Alice"}, dialect=Dialect.MYSQL)
        rows = _fetch_all(db, result)
        mapper = create_mapper(Employee)
        employees = mapper.map_rows(rows)
        assert len(employees) == 1
...
class TestMultipleParams:
    ...
    def test_multiple_conditions(self, db: Any) -> None:
        ...
        result = parse_sql(sql, {"dept_id": 10, "name": "Alice"}, dialect=Dialect.MYSQL)
        rows = _fetch_all(db, result)
        mapper = create_mapper(Employee)
        employees = mapper.map_rows(rows)
        assert len(employees) == 1
    ...
    def test_in_clause_with_regular_param(self, db: Any) -> None:
        ...
        result = parse_sql(sql, {"dept_id": 10, "ids": [1, 3]}, dialect=Dialect.MYSQL)
        rows = _fetch_all(db, result)
        mapper = create_mapper(Employee)
        employees = mapper.map_rows(rows)
        assert len(employees) == 2
...
class TestNullHandling:
    ...
    def test_non_removable_null_param(self, db: Any) -> None:
        ...
        result = parse_sql(sql, {"dept_id": None}, dialect=Dialect.MYSQL)
        rows = _fetch_all(db, result)
        # MySQL の WHERE dept_id = NULL は0件（IS NULL が必要）
        assert len(rows) == 0

    def test_select_null_field(self, db: Any) -> None:
        """NULL フィールドを持つレコードのマッピング."""
        sql = "SELECT * FROM employees WHERE id = /* $id */999"
        result = parse_sql(sql, {"id": 4}, dialect=Dialect.MYSQL)
        rows = _fetch_all(db, result)
        mapper = create_mapper(Employee)
        employees = mapper.map_rows(rows)
        assert len(employees) == 1
...
class TestDialectLikeEscape:
    ...
    @pytest.fixture
    def db_with_special_names(self, mysql_conn: Any) -> Any:
        """特殊文字を含む名前のテストデータ."""
        cur = mysql_conn.cursor()
    ...
    def test_like_escape_percent(self, db_with_special_names: Any) -> None:
        """% を含む値を LIKE 検索."""
        search = escape_like("10%", Dialect.MYSQL)
        sql = "SELECT * FROM products WHERE name LIKE /* $pattern */'%' ESCAPE '#'"
        result = parse_sql(sql, {"pattern": f"{search}%"}, dialect=Dialect.MYSQL)
    ...
    def test_like_escape_underscore(self, db_with_special_names: Any) -> None:
        """_ を含む値を LIKE 検索."""
        search = escape_like("Product_", Dialect.MYSQL)
        sql = "SELECT * FROM products WHERE name LIKE /* $pattern */'%' ESCAPE '#'"
        result = parse_sql(sql, {"pattern": f"{search}%"}, dialect=Dialect.MYSQL)
        rows = _fetch_all(db_with_special_names, result)
        assert len(rows) == 1
